...
import adminAnim from "../../../animations/signin.json"; 
...
export default function AdminLoginPage() {
    ...
    try {
     ...
     if (res.ok) {
      ...
      if (res.status === 401 && errorMessage.toLowerCase().includes("unauthorized")) {
        setError("You are unauthorized to access this admin panel.");
      } else {
  ...
  const desktopAnimPositions = [
    { top: "0%", left: "20%", size: "700px" },
    { top: "0%", right: "-25%", size: "650px" },
    { top: "0%", left: "-25%", size: "650px" },
  ];
    ...
    <div className="relative flex items-center justify-center min-h-screen overflow-hidden bg-gray-50">
      ...
      <div className="hidden lg:block">
        {desktopAnimPositions.map((pos, i) => (
          ...
          >
            <Lottie animationData={adminAnim} loop={true} />
          </div>
        ))}
      </div>
      ...
      >
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            className="relative w-full py-3 lg:py-2 rounded-lg bg-primary font-semibold overflow-hidden group"
          >
