  ...
  return (
    <div className="bg-background min-h-screen text-dark relative">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {topDoctors.map((doc, idx) => (
              <div
                key={idx}
                className="group [perspective:1000px] transition-transform duration-500 hover:scale-105"
              >
                <div className="relative w-full h-64 rounded-xl shadow-lg border border-gray-200 bg-white [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] transition-transform duration-700">
                  {/* Front Side */}
                  <div className="absolute inset-0 flex flex-col items-center justify-center backface-hidden p-4">
                    ...
                    <p className="mt-2 text-gray-600 text-sm">
                      Patients:{" "}
                      <span className="font-semibold">{doc.patientCount || 0}</span>
                    </p>
                  </div>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-[#5DC4C7] to-cyan-600 text-white text-center p-4 rounded-xl [transform:rotateY(180deg)] backface-hidden">
                    ...
                    <button
                      onClick={() => router.push(`/patient/doctors/${doc.doctorId}`)}
      ...
      <footer className="bg-dark text-white py-6 sm:py-8 px-4 sm:px-6 text-center">
        <p className="text-sm sm:text-base">&copy; 2025 Teck Starck. All Rights Reserved.</p>
